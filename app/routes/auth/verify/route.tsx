...
// Simple in-memory cache to prevent duplicate sends
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const autoSend = url.searchParams.get("autoSend"); // Only send if explicitly requested

  // Don't auto-send email on every load - only when explicitly requested
  if (!email || autoSend !== "true") {
    return json({
      emailSent: false,
      error: null,
      email: email || null,
